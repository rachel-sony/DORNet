...
class DeformableConvRGB(nn.Module):
    def __init__(
        self,
        in_channels,
        out_channels,
        ...
        super(DeformableConvRGB, self).__init__()
    ...
    def forward(self, input_feat, inter, fea):
        ...
        weight = weight.view(
            b * self.out_channels, self.in_channels, self.kernel_size[0], self.kernel_size[1]
        ).contiguous()
        ...
        out = modulated_deform_conv2d(
            input_feat.contiguous(), offset, mask, weight, self.bias, self.stride, self.padding, self.dilation, b, b
        )
